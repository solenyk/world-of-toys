...
public interface CartItemRepository extends JpaRepository<CartItem, CartItemId> {
    @Query("SELECT new com.kopchak.worldoftoys.dto.cart.CartItemDto(p.name, p.slug, p.price * c.quantity, " +
            "c.quantity) FROM CartItem c JOIN c.id.product p WHERE c.id.user.email = :email")
    Set<CartItemDto> findAllCartItemDtosByUserEmail(@Param("email") String email);

    @Query("SELECT SUM(p.price * c.quantity) FROM CartItem c JOIN c.id.product p WHERE c.id.user.email = :email")
